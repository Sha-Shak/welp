...
function ContactCard({ room }) {
  ...
  const otherUserId =
    room.user_id1 === currentUser.id ? room.user_id2 : room.user_id1;
            ...
            <span className="border-2 border-gray-light w-full flex ml-1 items-center  h-auto bg-white font-normal rounded-md px-3 p-4 ">
              ...
              <div className="">
                <div className="px-3 text-black text-xl">
                  ...
                  {contact ? (
                    contact.type === "admin" ? (
                      <span className="bg-indigo/50 text-gray-dark text-xs font-semibold mr-2 ml-3 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800">
                        Admin
                      </span>
                    ) : null
                  ) : null}
